...
function DogSearchPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await axios.get(`/dogs/search?${params.toString()}`, {
        headers: {
  ...
  const handleClear = () => {
    setSearchParams({ id: "", name: "", trainingStatus: "", reserved: "" });
    setResults([]);
    setError("");
  };
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto animate-fadeIn transition-all duration-500">
      ...
      {/* Form Grid for search inputs */}
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 print:grid-cols-1 mb-6">
        <div>
          <Label htmlFor="id" className="dark:text-gray-200">Dog ID</Label>
          <Input
            id="id"
            name="id"
        ...
        <div>
          <Label htmlFor="trainingStatus" className="dark:text-gray-200">Training Status</Label>
          <Input
            id="trainingStatus"
            name="trainingStatus"
            placeholder="e.g. basic, intensive"
            value={searchParams.trainingStatus}
            onChange={handleChange}
            className="dark:bg-gray-900 dark:text-white dark:border-gray-700"
          />
        </div>
